...
import {
  splitMarkdownByChunks,
  splitMarkdownByHeadings,
  toManifestParts,
} from "./split-markdown.js";
...
export type PipelineInput = {
  jobId: string;
  sourcePath: string;
  workDir: string; // job working directory
...
function assertNotCancelled(hooks: PipelineHooks) {
...
export async function runPdfPipeline(
  ...
  const assetBudget =
    input.options.assetBudgetBytes ?? DEFAULTS.assetBudgetBytes;
  ...
  const ranges =
    pageCount != null && pageCount > maxPer
  ...
  const segments: SegmentResult[] = ranges.map((r, index) => ({
    index,
...
export async function runEpubPipeline(
  input: PipelineInput,
  hooks: PipelineHooks = {},
): Promise<PipelineOutput> {
  const warnings: string[] = [];
 ...
 */
export async function retryFailedPdfSegments(
  input: PipelineInput & { previousSegments: SegmentResult[] },
  hooks: PipelineHooks = {},
): Promise<PipelineOutput> {
  if (!input.mineruToken) throw new Error("未配置 MinerU Token");
  const warnings: string[] = [];
  ...
  for (const seg of segments) {
    if (seg.state === "done" && seg.fullMd) continue;
    assertNotCancelled(hooks);
    seg.state = "running";
  ...
  const anyDone = segments.some((s) => s.state === "done");
  const allDone = segments.every((s) => s.state === "done");
  if (!anyDone) throw new Error("重试后仍无成功页段");
  const status: JobStatus = allDone ? "succeeded" : "partial";
