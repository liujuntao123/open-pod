...
import type { ManifestPart } from "./types.js";
...
type HeadingHit = {
  level: 1 | 2;
  title: string;
  index: number; // start of heading line in source
  endLineIndex: number; // end of heading line
};

function findHeadings(md: string, levels: Array<1 | 2>): HeadingHit[] {
...
function cutByHeadings(md: string, hits: HeadingHit[]): SplitResult["parts"] {
 ...
 */
export function splitMarkdownByHeadings(md: string): SplitResult {
...
export function toManifestParts(
