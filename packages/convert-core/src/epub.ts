...
import type { EpubOutputMode } from "./types.js";
import { safeFilename, truncateError } from "./util.js";

const require = createRequire(import.meta.url);

function resolveEpub2mdBin(): string {
  try {
    const pkgJson = require.resolve("epub2md/package.json");
    const dir = path.dirname(pkgJson);
    return path.join(dir, "lib/bin/cli.cjs");
  } catch {
    // fallback: look near this package
    return "epub2md";
  }
}
...
export type EpubConvertResult = {
  fullMd: string;
  parts: Array<{ filename: string; title: string; content: string; order: number }>;
...
async function collectImages(
  root: string,
  budgetBytes: number,
): Promise<{ images: Map<string, Buffer>; bytes: number; capped: boolean; warnings: string[] }> {
  ...
  async function walk(d: string) {
    ...
    for (const e of entries) {
      ...
      if (!/\.(png|jpe?g|gif|webp|bmp|svg)$/i.test(e.name)) continue;
      if (capped) continue;
...
export async function convertEpub(params: {
  ...
  if (params.localize) argsBase.unshift("--localize");

  const mode = params.outputMode;
  const runModes: Array<"split" | "merge"> =
    mode === "both" ? ["split", "merge"] : mode === "merge" ? ["merge"] : ["split"];
  ...
  for (const m of runModes) {
    ...
    if (result.code !== 0) {
      throw new Error(
        `epub2md ${m} 失败 (code ${result.code}): ${result.stderr || result.stdout}`.slice(0, 800),
      );
    }
  }
  ...
  let fullMd = "";
  const parts: EpubConvertResult["parts"] = [];
  ...
  const { images, capped, warnings: imgWarn } = await collectImages(
    imgRoot,
    params.assetBudgetBytes,
  );
